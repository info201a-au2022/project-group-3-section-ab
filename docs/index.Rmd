---
title: "An Analysis of Abortion Trends in California"
author: "Kayla Gibbs (kgibbs27@uw.edu), Kristy Nhan (kristynh@uw.edu), Elizabeth Luna-Santos (elunas1@uw.edu)"
date: "11/16/2022"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, warning=FALSE, message=FALSE)
library(knitr)
```


## Summary Information



### Our table of aggregated data, grouping by the state of California.  
```{r echo=FALSE}
source("../source/single_table.R")
# split table into two sections for readability
kable(merged_table[1:5], caption = "Public opinions on abortions in California: proportions in favor of legalizing")
kable(merged_table[6:10], caption = "Average cost of abortions in California and average driving times to abortion clinics")
```
include a brief paragraph, describing why you included the table, and what information it reveals.


---

### Dataset 1: Abortion Knowledge and Attitudes Poll (Kaiser Family Foundation)
```{r echo=FALSE, fig.width=8, fig.height=4, fig.cap="Source: Henry J. Kaiser Family Foundation for Roper Center at Cornell University"}
source("../source/dataset3_graph.R")

```



---

### Dataset 2: Abortion-Related Services Funded by Medi-Cal, years 2014 to 2020 (Medi-Cal)
```{r echo=FALSE, fig.width=8, fig.height=4, fig.cap="Source: Medi-Cal and California’s Department of Health Care Services (DHCS)"}
source("../source/dataset3_graph.R")

```
 
 
 
---
 
### Dataset 3: Driving Times to Abortion Clinics in the US (The Pudding)
```{r echo=FALSE, fig.width=8, fig.height=4, fig.cap="Source: The Pudding (MIT)"}
source("../source/dataset3_graph.R")
line_graph
```
describing why you included the chart and what information it reveals, including notable observations and insights from the chart.



------
DELETE THIS LATER

## What is this file for?

* This file will eventually become your project report for **P02: Exploratory Data Analysis**. Specifically, you will write rmarkdown to report your exploratory data analysis.

* Please see Canvas for more details.

## ACCESSING THE /source DIRECTORY
### Example 1: Source some code and run a function
```{r}
# Example 1: Note relative path, which can be read: Up one
# directory(..), down into source (/source), and 
# then "source" an R file (data_access.R)
source("../source/data_access.R")
data_access_test()
```

### Example 2: Initialize a variable and then use it
```{r}
# Example 1: This function was "sourced" above
msg <- data_access_test(" Morgan!")
```

> `r msg` Hope you have a good day!! 

## ABOUT THIS FILE: 

This is an R Markdown document. Markdown is a simple formatting syntax for authoring HTML, PDF, and MS Word documents. For more details on using R Markdown see <http://rmarkdown.rstudio.com>.

When you click the **Knit** button a document will be generated that includes both content as well as the output of any embedded R code chunks within the document. You can embed an R code chunk like this:

```{r cars}
summary(cars)
```

## Including Plots

You can also embed plots, for example:

```{r pressure, echo=FALSE}
plot(pressure)
```

Note that the `echo = FALSE` parameter was added to the code chunk to prevent printing of the R code that generated the plot.
